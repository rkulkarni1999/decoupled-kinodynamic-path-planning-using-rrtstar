...
sys.path.append('/home/ankit/Documents/STUDY/RBE595/HW2a/amittal_p2a')
...
class quad_control:
    def __init__(self):
        ...
        # self.z_pid = pid(0.1, 2.0, 1.0,  0.1, 0.051.0, filter_tau, dt, minVal = minVel, maxVal=maxVel)

        # self.x_pid = pid(4.0, 0.1, 0.4, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.y_pid = pid(3.0, 0.1, 0.3, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        # self.z_pid = pid(5.0, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)



        self.x_pid = pid(1.0, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.y_pid = pid(1.0, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)
        self.z_pid = pid(1.0, 0.0, 0.1, filter_tau, dt, minVal = minVel, maxVal=maxVel)

        # NED velocity controller. EDIT GAINS HERE. KINDA DONE!!
        
        self.vx_pid = pid(1.0,  0.1, 0.05, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vy_pid = pid(4.0,  0.1, 0.1, filter_tau, dt, minVal = minAcc, maxVal=maxAcc)
        self.vz_pid = pid(20.0, 0.0, 0.05, filter_tau, dt, minVal = minAcc, maxVal = maxAcc)
    ...
    def step(self, X, WP, VEL_SP, ACC_SP):
        ...
        scipy.io.savemat('/home/ankit/Documents/STUDY/RBE595/HW2a/amittal_p2a/log/control.mat', loggedDict)
